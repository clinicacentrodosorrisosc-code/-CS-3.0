...
async function run() {
  ...
  console.log("Verificando se há registros na tabela...");
  ...
  if (data.length > 0) {
    console.log("Já existem registros. Não é necessário inserir.");
  } else {
    console.log("Tabela vazia. Tentando inserir...");
    const { error: insertError } = await supabase.from('kommo_tokens').insert({
