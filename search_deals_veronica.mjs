...
async function run() {
  console.log("Searching in deals...");
  const { data: d1 } = await supabase.from('deals').select('*').ilike('title', '%Verônica%');
  const { data: d2 } = await supabase.from('deals').select('*').ilike('notes', '%Verônica%');
  const combined = [...(d1 || []), ...(d2 || [])];
  if (combined.length > 0) {
    console.log("Found in deals:", JSON.stringify(combined, null, 2));
  } else {
    console.log("Not found in deals.");
  }
}
